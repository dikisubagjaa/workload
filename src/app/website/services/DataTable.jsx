...
export default function DataTable() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await fetch("/api/website/service", { cache: 'no-store' });
            const result = await res.json();
            if (!res.ok) {
                throw new Error(result?.msg || "Failed to fetch services");
            }
    ...
    const buildActionContent = (record) => (
        <div className="p-1">
            ...
            <div className="flex flex-col gap-2 px-1 pb-1">
                <Link href={`/website/services/edit/${record.service_id}`} className="inline-grid">
                    <Button
                        size="small"
                        type="default"
    ...
    const colomnDetail = () => [
        {
            ...
            render: (_, record) => (
                <Image alt="Image Cover" width={400} height={200} src={record.cover_url || "/static/images/icon-image.png"} className="w-14 h-14 object-cover border rounded-md" />
            ),
        },
        {
            title: 'Title',
        ...
        },
        {
            ...
            render: (_, record) =>
                <p className="text-sm">
                    {truncateText(record.subtitle, 50)}
                </p>,
        },
        {
            title: 'Description',
            ...
            render: (_v, r) => {
                ...
                return (
                    <Popover
                        trigger="click"
                        placement="left"
                        open={openActionFor === r.service_id}
                        onOpenChange={(v) => setOpenActionFor(v ? r.service_id : null)}
                        content={content}
                    >
        ...
        return (
            <ul className="flex flex-col gap-3">
                ...
                <li>
                    ...
                    <h3 className="text-sm">{record.subtitle}</h3>
                </li>
    ...
    return (
        <>
            <div className="flex justify-between flex-wrap gap-3 mb-6">
                ...
                <Link href="/website/services/create" className="w-full sm:w-auto">
            ...
            {loading ? (
                ...
                <Table
                    dataSource={filteredData.map(item => ({ ...item, key: item.service_id }))}
