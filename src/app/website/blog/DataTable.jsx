...
export default function DataTable() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await fetch("/api/website/blog", { cache: 'no-store' });
    ...
    const buildActionContent = (record) => (
        <div className="p-1">
            ...
            <div className="flex flex-col gap-2 px-1 pb-1">
                <Link href={`/website/blog/edit/${record.blog_id}`} className="inline-grid">
                    <Button
                        size="small"
                        type="default"
                ...
                <Popconfirm
                    ...
                    onConfirm={() => handleDelete(record.blog_id)}
                    okText="Yes"
                    cancelText="No"
                >
                    <Button
    ...
    const colomnDetail = () => [
        {
            ...
            render: (_, record) => (
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-semibold text-sm">{truncateText(record.title, 50)}</h3>
            ...
            render: (_v, r) => {
                ...
                return (
                    <Popover
                        trigger="click"
                        placement="left"
                        open={openActionFor === r.blog_id}
                        onOpenChange={(v) => setOpenActionFor(v ? r.blog_id : null)}
                        content={content}
                    >
    ...
    return (
        <>
            <div className="flex justify-between flex-wrap gap-3 mb-6">
                ...
                <Link href="/website/blog/create" className="w-full sm:w-auto">
            ...
            {loading ? (
                ...
                <Table
                    dataSource={filteredData.map(item => ({ ...item, key: item.blog_id }))}
